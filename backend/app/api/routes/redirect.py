...
async def _log_click(link_id: int, country: str, db: AsyncSession):
    """Background task: log a click without blocking the redirect."""
    async with db:
        click = Click(
            link_id=link_id,
            clicked_at=date.today(),
            country=country[:2] if country else "XX",
        )
        db.add(click)
        await db.commit()
...
async def redirect_link(
    ...
    if cached:
        ...
        background_tasks.add_task(
            _log_click_by_code, code, country, db
        )
    ...
    # --- Cache the URL ---
    await redis.setex(cache_key, settings.redis_ttl, link.original_url)
...
async def _log_click_by_code(code: str, country: str, db: AsyncSession):
    """Look up link_id from code, then log click."""
    result = await db.execute(select(Link).where(Link.short_code == code))
    link = result.scalar_one_or_none()
    if link:
        click = Click(
