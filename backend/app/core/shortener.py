import hashlib
import time
import string
import re


BASE62_CHARS = string.ascii_letters + string.digits  # a-z A-Z 0-9


def _sha256_to_int(data: str) -> int:
    """Hash a string with SHA256 and return as integer."""
    digest = hashlib.sha256(data.encode()).digest()
    return int.from_bytes(digest, byteorder="big")


def _int_to_base62(n: int, length: int = 7) -> str:
    """Convert integer to base62 string of fixed length."""
    result = []
    while n > 0:
        result.append(BASE62_CHARS[n % 62])
        n //= 62
    # Pad or trim to desired length
    code = "".join(reversed(result))
    return code[:length].ljust(length, BASE62_CHARS[0])


def generate_short_code(url: str, length: int = 7) -> str:
    """
    Generate a short code: base62-encode SHA256 of (url + timestamp).
    First 7 characters of the encoded hash.
    """
    seed = f"{url}{time.time_ns()}"
    n = _sha256_to_int(seed)
    return _int_to_base62(n, length)


def validate_custom_slug(slug: str) -> bool:
    """
    Validate custom slug: 3-20 chars, alphanumeric + hyphens only.
    No leading/trailing hyphens.
    """
    if not slug:
        return False
    pattern = r'^[a-zA-Z0-9][a-zA-Z0-9\-]{1,18}[a-zA-Z0-9]$|^[a-zA-Z0-9]{3,20}$'
    return bool(re.match(pattern, slug)) and 3 <= len(slug) <= 20
